...
def top_5_doctors_revenue(db: Session):
    ...
    stmt = stmt.group_by(Doctors.id, Doctors.name).order_by(func.sum(VisitsHistory.price).desc()).limit(5)
...
def avg_stay_by_ward(db: Session):
    ...
    stmt = stmt.where(Hospital.discharge_date.isnot(None)).group_by(Wards.id, Wards.category)
    return db.execute(stmt).all()
...
def clients_with_procedure_and_contra(db: Session):
    ...
    stmt = stmt.join(ClientsContraindications).join(Contraindications)
    stmt = stmt.join(VisitsHistory).join(Procedures)
    stmt = stmt.join(ProceduresContraindications,
        and_(Procedures.id == ProceduresContraindications.procedure_id,
...
def supplies_count_by_procedure(db: Session):
    ...
    ).join(ProceduresSupplies)
    stmt = stmt.group_by(Procedures.id, Procedures.name).order_by(func.count().desc())
    return db.execute(stmt).all()
...
def clients_with_analysis_no_payment(db: Session):
    ...
    stmt = stmt.join(PrescribedAnalysis).outerjoin(Payments, Payments.client_id == Clients.id)
...
def procedures_with_contra_clients(db: Session):
    ...
    ).join(ProceduresContraindications).join(ClientsContraindications)
...
def top_3_procedures_by_avg_payment(db: Session):
    ...
    ).join(VisitsHistory, and_(Payments.client_id == VisitsHistory.client_id, Payments.procedure_id == VisitsHistory.procedure_id))
...
def contraindications_stats(db: Session):
    ...
    ).outerjoin(ClientsContraindications)
...
def doctors_unique_clients_alt(db: Session):
...
def most_requested_procedures(db: Session):
    ...
    stmt = stmt.group_by(Procedures.id).order_by(func.count().desc())
    return db.execute(stmt).all()
...
def visits_count_per_doctor(db: Session):
...
def clients_for_specific_analysis(db: Session):
    stmt = select(
        Clients.id,
        Clients.name
    ).distinct().join(PrescribedAnalysis).join(Analysis)
    stmt = stmt.where(Analysis.name == 'Общий анализ крови')
    return db.execute(stmt).all()

from sqlalchemy import select, func
from app.models import Clients, Procedures, VisitsHistory, Doctors
...
def repeat_visit_messages(db: Session):
    stmt = (
        ...
        .select_from(VisitsHistory)
        .join(Clients, VisitsHistory.client_id == Clients.id)
        .join(Procedures, VisitsHistory.procedure_id == Procedures.id)
        .join(Doctors, VisitsHistory.doctor_id == Doctors.id)
        .where(VisitsHistory.doctors_report.like('%Необходима повторна%'))
